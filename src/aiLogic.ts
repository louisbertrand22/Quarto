 ...
 */
const wouldWin = (board: Board, row: number, col: number, piece: Piece): boolean => {
  // Temporarily place the piece
  const testBoard = board.map(r => [...r]);
  testBoard[row][col] = piece;
  
  // Check if this creates a winning line
  return checkWinningLine(testBoard, row, col);
};

/**
 * Check if a newly placed piece creates a winning line
 */
const checkWinningLine = (board: Board, row: number, col: number): boolean => {
 ...
 */
export const aiChoosePosition = (board: Board, piece: Piece): { row: number; col: number } => {
  ...
  for (const pos of emptyPositions) {
    if (wouldWin(board, pos.row, pos.col, piece)) {
      return pos;
    }
  }
 ...
 */
export const aiChoosePiece = (board: Board, availablePieces: Piece[]): Piece => {
  const emptyPositions: { row: number; col: number }[] = [];
  ...
  const safePieces = availablePieces.filter(piece => {
    ...
    return !emptyPositions.some(pos => wouldWin(board, pos.row, pos.col, piece));
  });
