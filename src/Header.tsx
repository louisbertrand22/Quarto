        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <nav className="flex items-center gap-1 sm:gap-2 md:gap-3">
            {showNavigation && onHomeClick && (
              <button
                onClick={onHomeClick}
                className={`flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg transition-colors ${currentView === 'game' ? 'bg-white/20 text-white' : 'text-white hover:bg-white/10'}`}
                title={t.header.home}
              >
            ...
            {showNavigation && onStatsClick && (
              <button
                ...
                className={`flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg transition-colors ${currentView === 'stats' ? 'bg-white/20 text-white' : 'text-white hover:bg-white/10'}`}
                title="Statistiques"
              >
                <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 sm:px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
            ...
            {user ? (
              <div
                ...
                className={`flex items-center gap-2 cursor-pointer px-2 sm:px-3 py-1.5 rounded-full border transition-colors ${currentView === 'profile' ? 'bg-white text-indigo-700 border-white' : 'bg-white/10 text-white border-white/20 hover:bg-white/20'}`}
              >
                <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-bold text-sm ${currentView === 'profile' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700'}`}>
              ...
              <button
                onClick={handleLogin}
                className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 bg-indigo-500 hover:bg-indigo-400 text-white rounded-full font-semibold shadow-sm transition-all border border-indigo-400/30"
              >
